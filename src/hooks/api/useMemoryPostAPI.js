...
export const memoryPostQueryKeys = {
    all: ["memory-post"],
    recent: (limit) => [...memoryPostQueryKeys.all, "recent", limit],
    timeOrder: (page, limit, order) => [...memoryPostQueryKeys.all, "time-order", { page, limit, order }],
    detail: (postId) => [...memoryPostQueryKeys.all, postId],
    familyMembers: (postId) => [...memoryPostQueryKeys.detail(postId), "family-members"],
};
// API 함수들
const memoryPostAPI = {
    // 최근 게시글 조회
    getRecentPosts: async (limit = 10) => {
        const response = await fetch(`${API_BASE_URL}/memory-post/recent?limit=${limit}`);
        return response.json();
    },
    // 시간순 게시글 조회
    getPostsByTimeOrder: async (params = {}) => {
        const { page = 1, limit = 20, order = "desc" } = params;
        const response = await fetch(`${API_BASE_URL}/memory-post/time-order?page=${page}&limit=${limit}&order=${order}`);
        return response.json();
    },
    // 게시글 단일 조회
    ...
    // 게시글 참여 가족 조회
    getPostFamilyMembers: async (postId) => {
        const response = await fetch(`${API_BASE_URL}/memory-post/${postId}/family-members`);
        return response.json();
    },
    // 게시글 생성
...
export const useRecentMemoryPosts = (limit = 10) => {
    return useQuery({
...
// 시간순 게시글 조회
export const useMemoryPostsByTimeOrder = (params = {}) => {
    return useQuery({
        queryKey: memoryPostQueryKeys.timeOrder(params.page, params.limit, params.order),
...
// 게시글 단일 조회
export const useMemoryPost = (postId) => {
    return useQuery({
...
// 게시글 참여 가족 조회
export const useMemoryPostFamilyMembers = (postId) => {
    return useQuery({
...
export const useOptimisticUpdateMemoryPost = () => {
    ...
    return useMutation({
        ...
        onMutate: async (newData) => {
            ...
            // 이전 데이터 스냅샷
            const previousPost = queryClient.getQueryData(memoryPostQueryKeys.detail(newData.postId));
            // 낙관적 업데이트
            queryClient.setQueryData(memoryPostQueryKeys.detail(newData.postId), (old) => {
                if (!old?.data)
                    return old;
                return {
                    ...old,
        ...
        onError: (err, newData, context) => {
            ...
            if (context?.previousPost) {
                queryClient.setQueryData(memoryPostQueryKeys.detail(newData.postId), context.previousPost);
            }
        },
