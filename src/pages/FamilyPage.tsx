...
import ReactFlow, { Controls, Node, Edge, NodeProps, Handle, Position } from "reactflow";
...
// 커스텀 노드 컴포넌트
const CustomNode: React.FC<NodeProps<{ name: string; imageUrl?: string }>> = ({ data }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '5px', boxSizing: 'border-box', border: '1px solid transparent', borderRadius: '4px' }}>
      <Handle type="target" position={Position.Top} style={{ background: '#555', width: '8px', height: '8px', borderRadius: '4px' }} />
      <img
      ...
      />
      <div style={{ marginTop: 5, fontSize: 12, fontWeight: 'bold' }}>{data.name}</div>
      <Handle type="source" position={Position.Bottom} style={{ background: '#555', width: '8px', height: '8px', borderRadius: '4px' }} />
    </div>
  );
};
...
const HubNode: React.FC<NodeProps> = () => {
  return (
    <div style={{
      ...
      <Handle type="target" position={Position.Top} style={{ top: '-4px', background: 'transparent', border: 'none' }} />
      <Handle type="source" position={Position.Bottom} style={{ bottom: '-4px', background: 'transparent', border: 'none' }} />
    </div>
  );
};
...
const familySamplesGenerated: FamilySample[] = Array.from({ length: 5 }, (_, i) => i + 1).flatMap(childCount => [
  {
    label: `아빠, 자녀 ${childCount}명`,
    value: `father_children${childCount}`,
    members: [
      { id: "p1", name: "아빠", role: "parentMale" as 'parentMale' },
      ...Array.from({ length: childCount }, (_, j) => ({ id: `c${j+1}`, name: `자녀${j+1}`, role: (j % 2 === 0 ? 'childMale' : 'childFemale') as 'childMale' | 'childFemale' }))
    ]
  },
  {
...
const familySamples: FamilySample[] = [sonOnlySample, ...familySamplesGenerated];

function getNodesAndEdges(sample: FamilySample): { nodes: Node[]; edges: Edge[] } {
  ...
  if (parents.length > 0 && children.length > 0) {
    ...
    parents.forEach((p, i) => {
      ...
      node.position = { x: parentStartX + i * parentHorizontalSpacing - NODE_WIDTH_ESTIMATE / 2, y: PARENT_Y };
    });
    ...
    children.forEach((c, i) => {
      ...
      node.position = { x: childrenStartX + i * childHorizontalSpacing - NODE_WIDTH_ESTIMATE / 2, y: CHILD_Y };
    });
    ...
    if (parents.length === 2) {
      ...
      reactFlowNodes.push({
        id: hubNodeId,
        data: {},
        position: { x: X_CENTER_OFFSET - HUB_NODE_WIDTH_ESTIMATE / 2, y: HUB_Y },
        type: 'hubNode',
      });
      // 부모 -> HUB 엣지
      parents.forEach(p => {
        reactFlowEdges.push({ id: `e-${p.id}-${hubNodeId}`, source: p.id, target: hubNodeId, type: edgeType, animated: false, style: edgeStyle });
      });
      // HUB -> 자녀 엣지
      children.forEach(c => {
        reactFlowEdges.push({ id: `e-${hubNodeId}-${c.id}`, source: hubNodeId, target: c.id, type: edgeType, animated: false, style: edgeStyle });
      });
    } else if (parents.length === 1) {
      // 단일 부모 -> 모든 자녀 엣지
      parents[0] // 부모 노드 위치는 위에서 이미 설정됨
      children.forEach(c => {
        reactFlowEdges.push({ id: `e-${parents[0].id}-${c.id}`, source: parents[0].id, target: c.id, type: edgeType, animated: false, style: edgeStyle });
      });
    }

  } else if (parents.length > 0) { // 부모만 있는 경우
    const parentHorizontalSpacing = calculateDynamicSpacing(parents.length);
    const parentTotalWidth = (parents.length - 1) * parentHorizontalSpacing;
    const parentStartX = X_CENTER_OFFSET - parentTotalWidth / 2;
    parents.forEach((p, i) => {
      const node = reactFlowNodes.find(n => n.id === p.id)!;
      node.position = { x: parentStartX + i * parentHorizontalSpacing - NODE_WIDTH_ESTIMATE / 2, y: (PARENT_Y + CHILD_Y) / 2 };
    });
    ...
    // }
  } else if (children.length > 0) { // 자식만 있는 경우 (형제자매)
    const childHorizontalSpacing = calculateDynamicSpacing(children.length);
    const childrenTotalWidth = (children.length - 1) * childHorizontalSpacing;
    const childrenStartX = X_CENTER_OFFSET - childrenTotalWidth / 2;
    children.forEach((c, i) => {
      const node = reactFlowNodes.find(n => n.id === c.id)!;
      node.position = { x: childrenStartX + i * childHorizontalSpacing - NODE_WIDTH_ESTIMATE / 2, y: (PARENT_Y + CHILD_Y) / 2 };
    });
    if (children.length > 1) {
      for (let i = 0; i < children.length - 1; i++) {
        reactFlowEdges.push({ id: `e-${children[i].id}-${children[i+1].id}`, source: children[i].id, target: children[i+1].id, type: edgeType, animated: false, style: edgeStyle });
      }
    }
  }
  ...
  if (members.length === 1 && reactFlowNodes.length === 1) {
      ...
      node.position = { x: X_CENTER_OFFSET - NODE_WIDTH_ESTIMATE / 2 , y: (PARENT_Y + CHILD_Y) / 2 };
  }
...
export function FamilyPage() {
  ...
  return (
    <div style={{ position: "relative", width: 390, height: 600, borderRadius: 20, overflow: "hidden" }}>
      <img
        src={bgTree}
        alt="family tree"
        style={{
      ...
      <div style={{ position: "absolute", top: 60, left: 30, zIndex: 2 }}>
        <span style={{ fontWeight: "bold" }}>Our Family : </span>
        <span>{selected.label}</span>
      </div>
      <div style={{ position: "absolute", top: 100, left: 30, zIndex: 2, width: 'calc(100% - 60px)' }}>
        <select
          value={selected.value}
          onChange={e => {
            const found = familySamples.find(f => f.value === e.target.value);
            setSelected(found!);
          }}
      ...
      <div style={{ position: "absolute", top: 150, left: 0, width: '100%', height: 400, zIndex: 1 }}>
        <ReactFlow
          key={selected.value}
          nodes={nodes}
          edges={edges}
