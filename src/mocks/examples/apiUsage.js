...
export const familyExamples = {
    ...
    // 가족 참여
    async joinFamily(familyCode, nickname) {
...
export const memoryPostExamples = {
    ...
    // 게시글 생성
    async createMemoryPost(title, content, images) {
    ...
    // 특정 게시글 조회
    async getMemoryPostById(postId) {
        const response = await fetch(`${API_BASE_URL}/memory-post/${postId}`);
        const result = await response.json();
        return result;
    },
    // 게시글 수정
    async updateMemoryPost(postId, title, content) {
        const formData = new FormData();
        if (title)
            formData.append("title", title);
        if (content)
...
export const commentExamples = {
    ...
    async getComments(postId, page = 1, limit = 10) {
        const response = await fetch(`${API_BASE_URL}/memory-post/${postId}/comment?page=${page}&limit=${limit}`);
        const result = await response.json();
        return result;
    },
    // 댓글 삭제
    async deleteComment(postId, commentId) {
        const response = await fetch(`${API_BASE_URL}/memory-post/${postId}/comment/${commentId}`, {
            method: "DELETE",
        });
        const result = await response.json();
        return result;
...
export const usageExamples = {
    ...
    async createPostAndComment() {
        try {
            // 1. 게시글 생성
            const postResult = await memoryPostExamples.createMemoryPost("우리 가족 여행", "오늘 정말 즐거운 하루였어요!", []);
            if (postResult.success) {
                console.log("게시글 생성 성공:", postResult.data);
                // 2. 댓글 작성
                const commentResult = await commentExamples.createComment(postResult.data.id, "정말 즐거워 보여요!");
