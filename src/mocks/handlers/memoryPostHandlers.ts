...
// Mock 데이터 생성 함수들
const createMockMemoryPost = (overrides?: Partial<MemoryPost>): MemoryPost => {
...
const createMemoryPost = http.post(
  "/api/v1/memory-post",
  async ({ request }) => {
    ...
    const response: BaseApiResponse<MemoryPost> = {
      code: 201,
...
const getRecentPosts = http.get("/api/v1/memory-post/recent", ({ request }) => {
  ...
  const recentPosts = [
    createMockMemoryPost({
      ...
      title: "가족 여행 추억",
      content:
        "오늘 가족들과 함께 여행을 다녀왔습니다. 정말 즐거운 시간이었어요!",
...
const getPostsByTimeOrder = http.get(
  "/api/v1/memory-post/time-order",
  ({ request }) => {
    const url = new URL(request.url);
...
const getMemoryPostById = http.get(
  "/api/v1/memory-post/:postId",
  ({ params }) => {
    const { postId } = params;
    const post = createMockMemoryPost({ postId: parseInt(postId as string) });
...
const getPostFamilyMembers = http.get(
  "/api/v1/memory-post/:postId/family-members",
  () => {
    ...
    const response: BaseApiResponse<FamilyMember[]> = {
      code: 200,
...
const updateMemoryPost = http.put(
  "/api/v1/memory-post/:postId",
...
const deleteMemoryPost = http.delete(
  "/api/v1/memory-post/:postId",
  ({ params }) => {
    const { postId } = params;

    const response: BaseApiResponse<{ deletedPostId: string }> = {
      code: 200,
      message: "게시글이 성공적으로 삭제되었습니다.",
      data: { deletedPostId: postId as string },
    };
